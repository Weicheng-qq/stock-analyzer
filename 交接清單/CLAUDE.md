...
| 用途 | 美股來源 | 台股對應官方來源 |
|---|---|---|
| **個股即時報價**（自選股/公司頁面股價跳動） | Yahoo Finance即時 | **`mis.twse.com.tw`（TWSE盤中資訊系統，2026-08-26改用）**——Yahoo對.TW/.TWO報價有延遲，開盤時特別明顯（使用者回報「不動+錯誤」），改用證交所自家看盤網頁用的免費端點，接近零延遲。`ex_ch=tse_{代碼}.tw`(上市)/`otc_{代碼}.tw`(上櫃)，支援`\|`分隔一次查多檔。查無資料才退回Yahoo |
