...
只回傳 JSON，不要任何其他文字、不要 markdown：
{
 ...
 "stance":"⚠️ 若上一步 why 寫的是「今日無明確公司消息」，這裡必須給 'neu'，
   不可以因為股價漲跌去反推正面或負面（那是先有結論再找理由，不是判讀）。
   有明確消息時才依消息內容本身判斷 pos/neu/neg。",
...
// ⚠️⚠️ 2026-09-05 修正：這條規則本身就沒攔到「今日無明確公司消息」——
//   那正是提示詞裡指定給 AI「找不到原因時」要用的固定句(見下方 aiNewsRead prompt)，
//   但這裡的正規表達式列的是另外幾種措辭，兩邊沒對齊。
//   實測：藥華藥(6446) why="今日無明確公司消息"、meaning=""，卻被放進「今天最值得
//   注意的事情」，使用者點進去看到的是一則沒有內容的「重點」，白白浪費時間。
const NO_CONTENT = /無明確相關新聞|無相關新聞|資訊不足|未提供|今日無明確公司消息|無明確公司消息/;
