...
| 用途 | 美股來源 | 台股對應官方來源 |
|---|---|---|
| **個股即時報價**（自選股/公司頁面股價跳動） | Yahoo Finance即時 | **`mis.twse.com.tw`（TWSE盤中資訊系統，2026-08-26改用）**——Yahoo對.TW/.TWO報價有延遲，開盤時特別明顯（使用者回報「不動+錯誤」），改用證交所自家看盤網頁用的免費端點，接近零延遲。`ex_ch=tse_{代碼}.tw`(上市)/`otc_{代碼}.tw`(上櫃)，支援`\|`分隔一次查多檔。查無資料才退回Yahoo。⚠️⚠️ **截至2026-08-28此問題仍未真正解決**（見第6.21節），已改用`no-store`完全停用快取並加上「最後更新HH:MM:SS（資料源）」時間戳診斷工具，但根因仍未確認
| 個股基本資料/財報 | Yahoo Finance | **TWSE OpenAPI**（`openapi.twse.com.tw`）、櫃買 TPEx OpenAPI |
...
### 6.12 台股官方 API 全被 403（⚠️ 2026-08-29 已證實此結論**部分錯誤**）
- **錯**：`/api/proxy` 抓 TWSE/TPEx/MOPS 官方 API 全被 403 擋。
- **當時解法**：改用內建靜態清單（`TW_ALL` 1983 檔）+ Yahoo + FinMind 免費 API。
- **⚠️⚠️ 2026-08-29 重大更正**：實測發現 **`openapi.twse.com.tw` 與 `www.tpex.org.tw/openapi`
  完全可用**（從本機與 GitHub Actions 呼叫皆 HTTP 200）。先前的 403 應該是**從 Vercel 機房 IP
  呼叫時才被擋**，不是這些來源本身不能用。可用且極有價值的端點：
  - `openapi.twse.com.tw/v1/opendata/t187ap06_L_ci` → **一次請求拿到全部 1035 家上市公司的季度損益表**
  - `www.tpex.org.tw/openapi/v1/mopsfin_t187ap06_O_ci` → 上櫃 874 家同上
  - `t187ap04_L` / `mopsfin_t187ap04_O` → 每日重大訊息（可篩出法說會公告）
- **教訓（與 6.19 同一條）**：不要把「單一情境（Vercel 機房 IP）的失敗」推論成「整個來源都不能用」，
  更不要把這種結論寫死進交接文件——這條錯誤結論讓後續每次都直接放棄嘗試官方來源，
  繞遠路用了 FinMind 與人工建置，實際上官方一個請求就能拿到全市場資料。
...
### 6.19 「TWSE全部被擋」的結論下得太快
- **錯**：先前交接記錄寫死「台股官方API全被403擋」，之後每次都直接放棄嘗試TWSE官方來源，改用Yahoo（結果Yahoo對台股有延遲，造成開盤股價bug）。
- **解法**：403是針對`openapi.twse.com.tw`，但TWSE自家看盤網頁用的`mis.twse.com.tw`完全可以直接call、無需金鑰、幾乎零延遲。**教訓：同一個網域下不同子網域/路徑的行為可能完全不同，遇到「被擋」時應該針對具體URL逐一測試，不要把單一測試結果推論成整個官方來源都不能用。**

### 6.20 派給研究agent的公司名稱沒先查證代碼
- **錯**：批次檢查Q2法說會缺口時，把股票代碼2388（實際是威盛電子/VIA）誤標成「全景軟體」（全景軟體實際代碼是8272）派給研究agent，導致agent查證了錯誤的公司。
- **解法**：**任何要交辦研究/更新任務前，先用`grep`/程式碼從`TW_ALL`或現有`IR_CALL_SUMMARY`常數查證代碼↔公司名稱對應，不要憑記憶或直覺輸入公司名稱**。好在該次agent本身有查證意識、主動回報「代碼與名稱不符」示警，才沒有把錯誤數據寫進網站——但這是agent盡責，不代表可以省略事前查證。

### 6.25 Yahoo 報價用錯 range → 當日漲跌幅變成三個月漲跌幅（2026-08-30）
- **錯**：新增每日功能時，用 `v8/finance/chart/{sym}?range=3mo` 一次取報價與均線。
  但 `range=3mo` 時 **`previousClose` 是 undefined**，程式退回 `chartPreviousClose`——
  那是**三個月前**的價格。實測 **GOOGL 正確 +1.74% 卻算成 -11.16%**、台積電 +0.41% 算成 +5.45%。
- **解法**：**當日漲跌一定要用 `range=1d`**（網站既有自選股本來就是這樣寫的，照抄即可）。
  需要均線/量能就另外發一個 `range=3mo` 請求，兩者分開。
- **教訓**：顯示錯誤的股價漲跌幅會直接摧毀使用者信任，比功能缺少更糟。
  加新功能時，**同一種資料要沿用站上既有的取法**，不要自己另想一套。

### 6.26 Yahoo 新聞搜尋對台股代碼回傳完全無關的內容（2026-08-30）
- **錯**：用 `v1/finance/search?q=2330.TW` 抓台積電新聞，實際回傳的是
  **Tillamook 乳製品公司的更正啟事**等毫無關聯的新聞。若餵給 AI，會產出
  「台積電受乳製品消息影響」這種荒謬結論，直接違反「以事實為依據」鐵則。
- **解法**：①台股先試美股 ADR 代碼（`TW_US_EQUIV`，2330→TSM）②用 Yahoo 回傳的
  `relatedTickers` 欄位過濾，只留真的包含本檔股票的新聞 ③**查無相關新聞就不給新聞分數，
  不硬湊**。
- **教訓**：外部 API 回傳 200 不代表資料正確。**餵給 AI 之前必須先驗證資料本身的相關性**，
  否則 AI 會很有信心地根據錯誤素材產出錯誤結論。

### 6.23 抓公司官網：純 fetch 抓到空殼、以及三種不同的「被擋」（2026-08-30）
自動化要達到人工建置的法說會品質，就必須抓公司官網逐字稿。過程中遇到**三種不同的失敗**，
每種解法都不一樣，接手者務必分清楚：
- **JS 動態渲染**：現代 IR 網站的文件連結是 JavaScript 載入後才出現，純 fetch 拿到空殼。
  實測 NVIDIA 純 fetch 得到 **0 個 PDF**、用無頭瀏覽器得到 **10 個含逐字稿**。
  → 解法：Playwright 無頭瀏覽器（`scripts/lib/ir-transcript.mjs`）。
- **擋非瀏覽器請求**：台積電對 Playwright 的 `ctx.request.get()` **一律回 403**，
  帶 Referer/Accept/Sec-Fetch-* 任何標頭都沒用，因為那不是真正由瀏覽器發出的請求。
  → 解法：改用 `page.evaluate()` 在頁面 JS 環境內 fetch，指紋與 cookie 完全一致。
- **CORS**：但頁面內 fetch 受同源政策限制，NVIDIA 的 PDF 在 `q4cdn.com` 跨網域會被擋。
  → 解法：**兩種方式互補，依序都試**（先頁面內 fetch，失敗再 ctx.request）。
- **真正的天花板**：約 19~21%（AAPL/MSFT/AVGO/亞培/金百利/洛克希德/康卡斯特）連無頭瀏覽器
  都被擋，屬 Akamai 等級防護（本檔 3.4 節記載的 TSLA/LLY 同一類）。關閉 HTTP/2 反而更糟。
  → 這些一律優雅退回 SEC 8-K 新聞稿，不要硬鑽。

### 6.24 用「有沒有拋錯」判斷外部指令是否存在（2026-08-30）
- **錯**：用 `pdftotext -v` 有沒有拋錯判斷 poppler 是否安裝。但 `pdftotext -v` 會把版本印到
  **stderr 並回傳非零結束碼**，導致明明裝好了卻一律判定「找不到」，台股法說會簡報整批被略過。
- **解法**：檢查錯誤訊息內容是否含版本字樣、以及 `e.code !== 'ENOENT'`（真的找不到執行檔）。
- **教訓**：Unix 工具的 `-v`/`--version` 不一定回傳 0，不能拿結束碼當「存在與否」的判斷依據。

### 6.22 vercel.json 加自訂欄位當註解 → 部署整個失敗（2026-08-29）
- **錯**：想在 `vercel.json` 的 headers 規則裡加 `"comment"` 欄位寫說明，導致 Vercel
  部署失敗，推送後正式站數分鐘仍停在舊版，一度誤以為只是部署慢。
- **解法**：`headers` 規則**只接受 `source` / `headers` / `has` / `missing` 四個欄位**，
  移除自訂欄位後立刻部署成功。說明文字要寫在別的檔案（本次寫進 `scripts/README.md`）。
- **教訓**：部署後若正式站遲遲不更新，先懷疑「設定檔不合法導致 build 失敗」，
  而不是預設「Vercel 比較慢」。

### 6.21 台股開盤股價問題連續三次修正未果（尚未解決，⚠️持續追蹤中）
- **狀態**：2026-08-26/27/28連續3個交易日，使用者回報台股開盤股價「不動、顯示前一交易日收盤價」。已嘗試三輪修正：①改用TWSE官方即時來源(取代Yahoo)、②fetch加4秒逾時、③即時報價徹底改用`no-store`停用所有快取層。**三輪都沒有確認真正解決**，因為每次驗證時都已經是收盤後，無法在9點開盤當下實測重現。
- **08-28關鍵新線索**：使用者說明是「9點到12點都錯，過12點才恢復正常」——完整3小時的異常窗口，這個時間尺度**目前無法用現有任何一個假設（load spike/快取/延遲）合理解釋**，需要列為優先待查方向。另外使用者的裝置是「三星手機點LINE連結安裝的app」，LINE內建瀏覽器有獨立於系統瀏覽器的快取機制，已請使用者測試「在瀏覽器開啟」但尚未取得結果。
- **教訓**：嘗試用`vercel logs`調閱事發當下的伺服器log以事後鑑識，**發現Vercel免費方案log保留時間極短（查不到20分鐘前的紀錄）**，這條路走不通。**下次遇到類似「難以重現的間歇性問題」，不要指望能事後從log找答案，要在修正的當下就想辦法讓系統自己留下證據**（本次做法：新增畫面上的「最後更新HH:MM:SS（資料源）」時間戳，讓使用者下次能直接截圖回報，取代反覆盲猜）。
- **下次接手務必先確認**：這個bug是否還在發生？如果還在，第一步是看使用者提供的「最後更新」時間戳截圖，不要重複已經試過的三個方向。
...
## 11.5 法說會資料自動更新系統（2026-08-29/30 建置，已取代人工逐間建置）

> **完整技術說明見 `scripts/README.md`。這裡只記原則層面的決定。**

### 11.5.1 架構
```
免費公開官方資料源 → 偵測新事件 → 只處理有新事件的公司 → 取得官方原始文件
→ Gemini 免費層分析 → commit 進 repo → Vercel 靜態檔 → 所有使用者共用
```
每天台灣時間 07:00 由 GitHub Actions 自動執行（public repo 免費且不計分鐘數）。

### 11.5.2 法說會內容的來源優先序（品質由高到低）
1. **公司官網逐字稿**（`ir-transcript.mjs`，無頭瀏覽器）——含管理層問答原話，
   品質等同人工建置。大型股取得率約 64%。入口是 `IR_QUARTERLY_PAGE` 人工整理的 1,270 筆網址。
2. **公司官網簡報**——同上來源，無問答內容
3. **台股 MOPS 法說會簡報**（`mops-earnings-call.mjs`）——近 2 個月約 884 家有上傳
4. **SEC 8-K 財報新聞稿**（`sec-press-release.mjs`）——有官方展望與分部營收，無問答
5. **官方季度損益表**（TWSE/TPEx OpenAPI、SEC XBRL）——只有數字

### 11.5.3 鐵則
- **只節錄不改寫**：所有原始文件送 AI 前只做「擷取相關段落」，原文一字不動，
  確保 AI 只能引用真實存在的內容。
- **提示詞要求保留措辭強度**：「比之前更強」不可淡化成「表現良好」；
  嚴禁把分析師的提問寫成公司的說法。
- **顯示過期數字比不顯示更糟**：抓到超過 400 天前的資料一律略過該公司，
  寧可保留既有人工內容。
- **前端排序以季度新舊決定**：自動摘要比人工版新超過 45 天才排前面，
  人工內容降為補充但完整保留。⚠️ 不能比季別數字（美股會計年度不對齊日曆季），要比期間結束日。

### 11.5.4 人工建置的定位已改變（2026-08-30 決定）
實測人工建置的 826 家台股中 **59% 已落後官方一季**——手做本質上追不上。
且自動化現在讀的就是人工當初讀的同一份文件（台積電事實涵蓋率 12/12）。
**因此不再需要每季全面重做人工建置。** 仍值得人工投入的只有三件事：
1. 約 20% 被機器人防護擋住的公司
2. **業務描述的人工更正**（如「2496 其實是補習班不是顯示器廠」）——自動化做不到
3. 抽查 AI 有沒有讀錯

## 11.6 「今日市場」每日回訪功能（2026-08-30 建置）

### 11.6.1 使用者的硬性前提
「**絕對不要修改目前既有的 App 架構**」——UI／頁面／導覽／個股頁／搜尋／新聞／財報／
AI／API 全部不可破壞。**因此一律採「新增」而非「修改」。**

### 11.6.2 新增功能時該照的既定模式（重要，日後加功能都照這個做）
`showGroup()` 裡的 `themeCard`(n=15)、`etfCard`(n=16)、`dailyCard`(n=17) 都是後來新增的功能。
標準做法只有四步，**零行既有邏輯會被更動**：
1. `#grpMenu` 加一顆按鈕 `<button data-g="N" onclick="pickGroup(N)">名稱</button>`
2. 新增一個 `<div class="card" id="xxxCard" style="display:none">`
3. `showGroup()` 加兩行（`const showXxx=(n===N)` 與對應的 display 切換）
4. 新增獨立的 JS 函式區塊，不動任何既有函式

若新卡片要出現在**個股頁**，則在 `cardGroup()` 加一行判斷（依卡片 `.ct` 標題文字歸組），
並在 `applyGroupAfterRender()` 加一行呼叫（用 try/catch 包住，失敗不影響既有流程）。

### 11.6.3 資料與成本
`scripts/daily-market.mjs` 由每日排程產生 `data/daily/latest.json` 與
`data/daily/scores/{代碼}.json`，前端只讀不算。
**五個評分面向中四個用官方數字直接計算**（市場情緒／技術面／基本面／估值），
**只有「新聞」用 AI** —— 既省額度又避免幻覺。

### 11.6.4 誠實揭露的兩個限制
- **通知只能做站內提示**（紅點＋橫幅），做不到瀏覽器推播：推播必須註冊 Service Worker，
  違反第 6.1 條鐵則。
- **本週事件不含美股個別公司財報日**：Yahoo 的 `quoteSummary`／`v7/finance/quote`
  現已需要 crumb 驗證（實測 401），免費拿不到，**不用猜的硬湊**。
  畫面上明確區分「官方公告」與「依慣例推算」。
